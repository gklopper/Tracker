...
class EventParser:
    ...
    def parse(self, xml_string):
        ...
        event = Event()
        event.event_type = self._get_text_value(event_xml, 'event_type')
    ...
    def _handle_story_events(self, event, event_xml):
        ...
        event.story_id = self._get_text_value(story_xml, 'id')

        if event.event_type == 'story_create':
            event.story_title = self._get_text_value(story_xml, 'name')
            event.story_type = self._get_text_value(story_xml, 'story_type')
        elif event.event_type == 'story_update' and self._tag_exists(story_xml, 'current_state'):
            event.story_state = self._get_text_value(story_xml, 'current_state')
            if event.story_state == 'accepted':
                event.accepted_date = self._get_date_value(story_xml, 'accepted_at')

    def _get_text_value(self, event_xml, field):
    ...
    def _get_date_value(self, event_xml, field):
        date_match = re.search(r'(\d\d\d\d)/(\d\d)/(\d\d)', self._get_text_value(event_xml, field))
